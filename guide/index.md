...
* `hooks`：因为Vue3中的`Composition API`。所以可以抽离出大量的原来页面中重用代码都放在这里。
* `logics`：如果一些代码是处理一些特殊逻辑的，它也不能算做工具类，也不是其他组件能重用的（或者说用的不多），这种代码放在这个文件夹中。比如初始化应用、主题修改等。
* `icons`：该文件夹是之前`svg`雪碧图插件的svg图片文件夹。
* `images`：非`svg`图片资源。
* `svg`：这里面也是一些`svg`图片，为什么没有放到`icons`里面，是因为`icons`可以通过`SvgIcon`组件展示，但是这里面的svg图片，有些是放在css中的。